...
程序设计训练（Python）OJ 课程项目，计划使用 FastAPI 异步后端与 Streamlit 前端。

当前仅完成开发环境和仓库初始化。题目管理、判题、用户、提交、日志和 AI 命题尚未实现，没有可运行的业务服务入口。
...
`uv sync` 会建立项目内的 `.venv`，不会将依赖安装到系统 Python。上述测试只验证基础依赖、异步 ASGI 调用和仓库忽略规则，不代表 OJ 功能已完成。
